#!/usr/bin/env python3
"""Generator-F: CPU matmul pooling of streaming generator data."""
...
def main():
    set_global_seed(680)
    configure_device("CPU", "Generator-F")
    ...
    model = build_model()
    model.compile(
        optimizer=tf.keras.optimizers.Adam(9e-4),
        loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
        metrics=["accuracy"],
        jit_compile=False,
    )

    callback = BatchEndCallback("generator-f_batch_end")
